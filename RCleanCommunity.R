...
Cleaned2018DataDateformatted <- Cleaned2018Data %>%
  #mutate(TimeFormatted = ifelse(is.na(Time), NA, substr(Time,1,nchar(Time)-3))) %>%
  mutate(DTFormatted = ifelse(is.na(DateTime), NA, sub("(.*)\\..*", "\\1", DateTime))) %>%
  mutate(DTFormatted = ifelse(is.na(DTFormatted), NA, sub("(.*)\\+.*", "\\1", DTFormatted))) %>%
  mutate(TimeFormatted = ifelse(is.na(Time), NA, sub("(.*)\\..*", "\\1", Time))) %>%
  mutate(TimeFormatted = ifelse(is.na(TimeFormatted), NA, sub("(.*)\\+.*", "\\1", TimeFormatted))) %>%
  mutate(DateAndTimeFormatted = ifelse(is.na(Date), NA, paste(as.character(Date), TimeFormatted, sep = " "))) %>%
  mutate(LastDTFormat = ifelse(is.na(DateAndTimeFormatted), DTFormatted, DateAndTimeFormatted)) %>%
  mutate(DateAndTime = as.POSIXct(LastDTFormat, format = "%Y-%m-%d %H:%M:%S")) %>%
  select(ID, Longitude, Latitude, ImageURL, ImageThumbnail,DateAndTime, Material, Item, Brand)

write.csv(Cleaned2018DataDateformatted, "CleanedDateDTFormatted2018.csv")

#LitteratiTrash <- fread("CleanedData.csv")



LitteratiTrashWinMaterials <- LitteratiTrash %>%
  mutate_all(funs(toupper)) %>%
  mutate_all(funs(gsub(" ", "", ., fixed = TRUE))) %>%
  group_by(Materials) %>%
  summarise(count = n())
  
LitteratiTrashWinItems <- LitteratiTrash %>%
  mutate_all(funs(toupper)) %>%
  mutate_all(funs(gsub(" ", "", ., fixed = TRUE))) %>%
  group_by(Items) %>%
  summarise(count = n())

LitteratiTrashWinBrands <- LitteratiTrash %>%
  mutate_all(funs(toupper)) %>%
  mutate_all(funs(gsub(" ", "", ., fixed = TRUE))) %>%
  group_by(Brands) %>%
  summarise(count = n())
...
Folder <- "/WinPhotos/"
for(photo in 1:nrow(LitteratiTrash)){
  download.file(url = paste(LitteratiTrash[photo,5], sep = ""), destfile = paste(Folder, LitteratiTrash[photo,1], ".jpg", sep = ""), mode = "wb" )
  Sys.sleep(0.1)
}
