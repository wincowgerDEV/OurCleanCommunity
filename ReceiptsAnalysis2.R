...
register_google(key = "AIzaSyAKeuM3boYBWdgOG8bRlYMon4saOtAJnFk") #To run geocode function
set_key(key = "AIzaSyAKeuM3boYBWdgOG8bRlYMon4saOtAJnFk") #To run google_find_place function
...
BrandLocationsMyData2 <- read.csv("BrandLocations_MyData2.csv") %>%
  rename(lat=lat, lon=lon, material=Materials, item= Items, brand=Brands, locationname=Location.Name, manualaddress=Manual.Address , manuallat = Manual.Lat, manuallon = Manual.Lon,  datetime=photo_timestamp, locationbrand=Location.Brand., photourl=photo_url, litterid=ï..id, notes=Notes) %>%
